package com.vahabghadiri.degreeviewsampleapp.degreeview
...
class DegreeView : View {
    ...
    private fun init() {
        ...
        paint.color = ContextCompat.getColor(context, R.color.gray0)
        paint.strokeWidth = context.dpToPx(2).toFloat()

        paintStartEnds = Paint()
        paintStartEnds.style = Paint.Style.FILL
        paintStartEnds.color = ContextCompat.getColor(context, R.color.gray0)
        paintStartEnds.strokeWidth = context.dpToPx(2).toFloat()

        paintMiddle = Paint()
        paintMiddle.style = Paint.Style.FILL
        paintMiddle.color = ContextCompat.getColor(context, R.color.gray11)
